...
export class AxiosCallError extends Error {
  constructor(axiosError: AxiosError) {
    ...
    this.message =
      `${axiosError.config.method?.toLocaleUpperCase()} ${
        axiosError.config.url
      }: ` +
      `Status: ${axiosError.response?.status || "-"} ${JSON.stringify(
...
export function InvalidPaypalToken(message?: string) {
  this.message = message;
  // Use V8's native method if available, otherwise fallback
  if ("captureStackTrace" in Error)
    Error.captureStackTrace(this, InvalidPaypalToken);
  else this.stack = new Error().stack;
}

InvalidPaypalToken.prototype = Object.create(Error.prototype);
InvalidPaypalToken.prototype.name = "InvalidPaypalToken";
InvalidPaypalToken.prototype.constructor = InvalidPaypalToken;

const isDevMode = process.env.ENV != "prod";
